...
// Testimonials — verbatim (lightly trimmed) quotes drawn from real STUDENTS
// cases above, reframed around each student's stated ambition for use as
// rotating social proof. Same content, same widget, on index.html and give.html.
const TESTIMONIALS = [
  { quote:"I'll be the first in my family to go to university. I want to become the teacher my younger siblings can look up to.", tag:"Aspiring Educator · 5 A’s in SPM, Form 6" },
  { quote:"I get by on RM5 a day and often skip breakfast, but I know exactly where I’m headed — medical school, then caring for patients as an anaesthesiologist.", tag:"Aspiring Medical Student · 7 A’s in SPM" },
  { quote:"Nine A's didn't come easy with everything going on at home, but I stayed focused. I want to become an accountant and ease things for my family.", tag:"Aspiring Accountant · 9 A’s in SPM, Top Science Student" },
  { quote:"I already skip meals to save for the bus home. I just want to focus on becoming an engineer without that constant worry.", tag:"Aspiring Mechanical Engineer · 8 A’s in SPM" },
  { quote:"Between caring for my family and my studies, I learned discipline early. I want to become the kind of teacher who never gives up on a struggling student.", tag:"Aspiring Teacher · 10 A’s in SPM, Math Olympiad Medalist" }
];

// Rotating student-quote carousel — auto-advances every 6s with clickable dots
// for manual override. rootId is a .hero-visual element already containing
// .hero-stat-quote / .hero-stat-tag / .hero-quote-dots (see index.html, give.html).
function initTestimonialCarousel(rootId){
  var root = document.getElementById(rootId);
  if(!root) return;
  var quoteEl = root.querySelector('.hero-stat-quote');
  var tagEl = root.querySelector('.hero-stat-tag');
  var dotsEl = root.querySelector('.hero-quote-dots');
  if(!quoteEl || !tagEl || !dotsEl) return;

  dotsEl.innerHTML = TESTIMONIALS.map(function(_, idx){
    return '<button type="button" class="hero-quote-dot" data-i="' + idx + '" aria-label="Show testimonial ' + (idx + 1) + '"></button>';
  }).join('');
  var dots = dotsEl.querySelectorAll('.hero-quote-dot');
  var current = 0;
  var timer;
  ...
  function paint(idx){
    ...
    setTimeout(function(){
      ...
      quoteEl.innerHTML = '<span class="quote-mark">&ldquo;</span>' + t.quote + '<span class="quote-mark">&rdquo;</span>';
  ...
  function startTimer(){
    timer = setInterval(function(){ paint((current + 1) % TESTIMONIALS.length); }, 6000);
  }
  dotsEl.addEventListener('click', function(e){
    var dot = e.target.closest('.hero-quote-dot');
    if(!dot) return;
    clearInterval(timer);
    paint(Number(dot.dataset.i));
    startTimer();
  });
  paint(0);
  startTimer();
